module.exports = (phoneNumber) => {
  ...
  // another 4 numbers
  // possible 'x' (or capital 'X'), followed by 1 to 6 numbers at the end
  const naPhoneRegex = /^(\+?1[\s.-])?\(?\d{3}\)?[\s.-]\d{3}[\s.-]\d{4}(\s(?:x|X)\d{1,6})?$/;

  // not as comprehensive as the North American Dialing Plan validator, but it's somewhat accurate, doesn't produce false negatives, and
  // allows for (most?) European local number formats (i.e. it doesn't have to be in E.164)
